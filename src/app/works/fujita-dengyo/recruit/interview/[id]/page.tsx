  ...
  '01': {
    ...
    questions: [
      ...
      },
      {
        ...
        answer: '大手メーカーとの協業プロジェクトに参加した経験が大きかったですね。様々な技能や個性を持った技術者の方々と一緒に仕事をすることで、多くの知識や技術を学ぶことができました。その経験で得たものを、今は後輩たちにフィードバックしています。',
    ...
    message: '保全業務と聞いて、すぐに仕事内容がわかる人は少ないと思いますが、社内の先輩方から丁寧に指導を受けることができ、様々なプロジェクト経験を通じて、知識や経験をしっかりと蓄えることができます。このような環境で働くことで、日々自分の成長を実感できると思います。ぜひ一緒に働きましょう！',
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${interview?.name || ''} インタビュー | フジタ電業株式会社`,
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
              {otherInterviews.map((item) => (
                <Link
                  key={item.id}
                  href={`/works/fujita-dengyo/recruit/interview/${item.id}`}
                  className="group"
                >
