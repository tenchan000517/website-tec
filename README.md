...
## 🚨 次世代Claude Codeへ - 最初に読むこと

### 現在のタスク: 東海部品工業サイト品質改善
...
1. **`HANDOVER-CRITICAL.md`** - 現在の状況と概要
2. **`QUALITY-IMPROVEMENT-HANDOVER.md`** - 詳細な作業手順（最重要）
3. TodoリストはTodoWriteツールで確認

**作業の進め方**:
```bash
# 1. QUALITY-IMPROVEMENT-HANDOVER.mdを読む
# 2. ベンチマーク画像を確認（C:\WEBサイトUI\東海部品工業株式会社\）
# 3. 1ページずつ修正（画像配置 → カラー修正 → レイアウト調整）
# 4. 各ページ修正後にコミット
# 5. すべてのページをブラウザで確認
```
